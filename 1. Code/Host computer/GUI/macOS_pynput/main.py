        ...
        for i in PdevKeys.keys():
            ...
            if key == i:
                if checkList(nowConf[PdevKeys[i]]) is True:     # 有加号
                    pp.press(decodeList(nowConf[PdevKeys[i]]))
                    ...
                    if nowConf[PdevKeys[i]] == 'up':
                        ...
                        pp.press(conv_ord(nowConf[PdevKeys[i]]))
                        for ec in ECKeys:       # Fix旋钮释放错误
                            if ec == key:
                                pp.release(conv_ord(nowConf[PdevKeys[i]]))
        ...
        for i in RdevKeys.keys():
            ...
            if key == i:
                if checkList(nowConf[RdevKeys[i]]) is False:        # 没有加号
                    pp.release(conv_ord(nowConf[RdevKeys[i]]))
...
def gui():
    ...
    layout = [
        ...
        [sg.Combo(Comobo, size=(8, 1), font=(FONT2), key='_nowList', default_value="DEFAULT"), sg.Btn('新建配置', key='_newConf', font=(FONT2), size=(6, 1)), sg.Btn('删除配置', key='_delConf', font=(FONT2), size=(6, 1)), sg.Btn('读取配置', key='_readConf', font=(FONT2), size=(6, 1)), sg.Btn('保存配置', key='_saveConf', font=(FONT2), size=(6, 1)), sg.Btn('配置说明', key='_aboutConf', font=(FONT2), size=(6, 1)), sg.Btn('关于', key='_about', font=(FONT2), size=(6, 1))]
