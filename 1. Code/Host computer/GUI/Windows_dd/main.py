...
def setKey(setKeyValue, newConfValue):
    ...
    while True:
        ...
        for i in PdevKeys.keys():
            ...
            if key == i:
                Pnk = nowConf[PdevKeys[i]]
                if checkList(Pnk) is True:     # 有加号
                    PressKeys(decodeList(Pnk))      # 有问题，暂时用回旧方案（已修复）
                    print(decodeList(Pnk))
                    # k.press_keys(decodeList(Pnk))

                elif checkList(Pnk) is False:      # 没有加号
                    if Pnk == 'up':
                        m.scroll(1, 0)
                    elif Pnk == 'down':
                        m.scroll(-1, 0)
        ...
        for i in RdevKeys.keys():
            ...
            if key == i:
                Rnk = nowConf[RdevKeys[i]]
                if checkList(Rnk) is False:        # 没有加号
                    for k in decodeList(Rnk):
                        dd_dll.DD_key(dd_dll.DD_todc(conv_ord(k.upper())), 2)
